...
We will perform our analysis of the \textsc{dv} in the framework of \textsc{fopt}
...
parameters, the strong coupling and an \textsc{ope} Wilson coefficient. Thus we
...
pinched weight it should have good suppression of \textsc{dv} contributions
...
implies a breakdown of \textsc{pt}. Furthermore, it bears the risk to be affected
...
We further tested the stability of dimension six and eight contributions to
...
dimension contribution of the \textsc{ope} is large. All in all the usage of
...
jump from \(0.2\) to \(0.95\) between the first two fitted moments. Note
...
values, a lower \(\alpha_s\) value and a \(\langle aGG \rangle_I\) numeric
...
\end{table}
For the weights with a monomial term \(x\) we only included fits, which make
use of the \textsc{bs}. Fits applying the \textsc{fopt} result in deviating
parameter values in comparison with fit results from weights without a monomial
term \(x\). This behaviour has already been illustrated in \cite{Beneke2012}
...
The values have a statistical error given by the fitting routine
\textsc{minuit} and a theoretical error. The main contribution from the
...
resulting asymmetric errors the larger value of the upper and lower error as a
