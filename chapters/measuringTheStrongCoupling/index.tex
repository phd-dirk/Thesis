...
\begin{wraptable}{l}{7cm}
  ...
  \begin{tabular}{@{\,}r <{\hskip 5pt} !{\foo} >{\raggedright\arraybackslash}p{5cm}}
    ...
    2015 & \cite{Boito2014}: Apply \textsc{dv} model to extract \(\alpha_s\) from \textsc{aleph} data. \\
    2016 & \cite{Pich2016}: Extract \(\alpha_s\) from \textsc{aleph} \(\tau\) data. Critic \textsc{dv} model. \\
...
based on the method developed in the early nineties by Braaten,
...
\subsubsection{\((1-x)^2(1+2x)\)}
\subsubsection{\((1-x)^3(1+3x)\)}
\subsubsection{\((1-x)^4(1+4x)\)}
