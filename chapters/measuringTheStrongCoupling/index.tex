...
\end{table}
To test the stability of the fitted values and have enough \textsc{dof} to
...
The first category contains the \textit{pinched weights without a monomial term x}. The
...
can further compare the \(\rho^{(6)}, \rho^{(8)}\) and \(\rho^{(10)}\) Wilson coefficients and
...
approximations
\begin{itemize}
  \item We used the \textsc{pt} expansion of the Adler function up to fifth
    order, including the educated guess of \(c_{5,1}\).
...
as it does not contain a monomial term \(x\). It is due to its polynomial structure
...
\begin{table}
  ...
  \begin{tabularx}{\textwidth}{ccYYYYY}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Table of our fitting values of \(\alpha_s(m_\tau^2), \rho^{(6)}, \rho^{(8)}\) and
    \(\rho^{(10)}\) for the cubic weight \(\omega(x)=(1-x)^3(1+3x)\) using
    \textsc{fopt} ordered by increasing \(s_{min}\). The errors are given in
    parenthesis after the observed value.}
...
\begin{figure}
  ...
  \caption{Graphic representation of the fitting values of
    \(\alpha_s(m_\tau^2)\) in the left and \(\rho^{(6)}, \rho^{(8)}\) and \(\rho^{(10)}\) in the
...
\(\alpha_s, \rho^{(6)}, \rho^{(8)}\) and \(\rho^{(10)}\) are in agreement with each other, except
for the fit with six momenta. The fit with a \(s_{min}=\SI{2.3}{\giga\eV}\) has
the lowest \(\chi^2=0.29\) and error on \(\alpha_s\), but takes slightly
...
\begin{equation}
  \alpha_s(m_\tau^2)=0.3302(40), \quad \rho^{(6)}=-0.52(11), \quad \rho^{(8)}=-0.58(22) \quad \text{and} \quad \rho^{(10)}=-1.00(45).
\end{equation}
...
The last fits of the pinched weights without a monomial term in \(x\) uses
the \textit{quartic weight} defined as \(\omega(x) \equiv (1-x)^4(1+4x)\). It
contains five fitting parameters (\(\alpha_s, \rho^{(6)}, \rho^{(8)}, \rho^{(10)}, \rho^{(12)}\)) and
...
\begin{table}
  ...
  \begin{tabularx}{\textwidth}{ccYYY}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Table of our fitting values of \(\alpha_s(m_\tau^2)\), and \(\rho^{(6)}\)
...
\begin{table}
  ...
  \begin{tabularx}{\textwidth}{ccYYY}
    ...
    \bottomrule
  \end{tabularx}
  \caption{Table of our fitting values of \(\alpha_s(m_\tau^2)\), and \(\rho^{(8)}\)
...
row, at \(s_{min}=\SI{2.8}{\giga\eV}\) has only one \textsc{dof} and
consequently high errors. The fit closest to the \(\chi^2/dof\) threshold then
yields the following parameter values
\begin{equation}
...
problematic of the monomial term \(x\), appearing in the weight function. The
...
\textsc{bs} we note that the fits have good \(\chi^2/dof\) values, although a
...
opposite sign. Consequently we observe some tension between
the single pinched
...
\begin{table}
  ...
  \end{adjustbox}
  \caption{Table of our fitting values of \(\alpha_s(m_\tau^2), \langle aGG
    \rangle_I, \rho^{(6)}, \rho^{(8)}\) and \(\rho^{(10)}\) for the optimal weight
...
problematic, we have chosen a fit closest, but below the
\(\chi^2/dof\) threshold. They are shown in \cref{table:fitCombinations}, which
...
further supported by this work. Consequently for fits including weights
...
other. The strong coupling as the \textsc{ope} contributions up to dimension
...
selected fits. We do not want to average over the errors of all compared fits,
as we do not know their correlations. Averaging over all errors would most
probably lead to an underestimated error.
