...
The \textit{two\-/point} function (or \textit{correlator}) is the vacuum
expectation value of two local fields. In the simplest case, for a scalar field $\phi$, it is
defined as
\begin{equation}
  \label{eq:simpleTwoPointFunction}
  \Omega \vert \phi(x) \phi(y) \vert \Omega \rangle.
\end{equation}
Here $\vert \Omega \rangle$ is the physical vacuum and the fields operators $\phi(x)$ and
$\phi(y)$ are time dependent Heisenberg operators, which can be translated to
its origin. The two\-/point function can be interpreted physically as the
amplitude of propagating single\-/ or multi\-/particles and their excitations.
The correlator can be expressed as a \textit{spectral decomposition}, which has
been discovered early on by \cite{Kallen1952,Lehmann1954} and is named \textit{Källén-Lehmann spectral
...
In the second quantisation we applied latter operators $a_o^\dagger$ on the free vacuum $\vert
0 \rangle$ to obtain single particle states, carrying a momentum $p$
\begin{equation}
...
$\vec X$ we can express the complete set of one\-/ and multiple\-/particle states as
\begin{equation}
...
\end{equation}
With these expressions in hand we can spectral decompose the two\-/point function
(\cref{eq:simpleTwoPointFunction}) 
\begin{equation}
  \begin{split}
...
$\vert X \rangle$ have to have positive energy eigenstates $p^0 > 0$. Consequently we can define the
\textit{spectral function} as
\begin{equation}
...
decomposition to the \define{FT}{Fourier transform} of the \textit{time\-/ordered two\-/point function}
\begin{equation}
...
where we made use of the translation invariance of the correlator\footnote{
  ...
  = \langle\Omega\vert e^{i\hat P y}\underbrace{e^{-i\hat P y}\phi(x)e^{i\hat P
...
\begin{equation}
  ...
  \begin{split}
    ...
    &= i\int\fdif{x}e^{-ixp}\int_0^\infty \dif q^2\, \rho(q^2) \int\fdif{p} \frac{i}{p^2 - q^2 - i\epsilon}e^{ipx}
  \end{split} 
\end{equation}
...
\end{equation}
The spectral function usually has a pole at one\-/particle states and a
branch cut above the multi\-/particle state threshold. The branch cut is of high
...
\textcolor{red}{correct sum up}
What have we done so far:
The scalar QCD two point function can
then be described by the spectrum of hadronic states. The correlator is then
related to an integral over the \textit{spectral function} $\rho(s)$ via the
\textit{Källén-Lehmann spectral representation} \cite{Kallen1952,Lehmann1954},
which is known since the early fifties
\begin{equation}
