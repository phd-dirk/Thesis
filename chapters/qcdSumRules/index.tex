...
from hadrons we need to bridge the quark\-/gluon picture with the hadron
picture. To do so we will introduce the framework of \textsc{qcdsr}.

We will start by setting up the foundations of strong interaction with
introducing the \textsc{qcd}\-/Lagrangian. The \textsc{qcd}\-/Lagrangian
describes the quark\-/gluon picture solely and is ruled by the abelian gauge
group $SU(3)$. The group has important implications on the strong coupling and
the applicability of \textsc{pt}. Next we will focus on the two\-/point
...
fields. We can use it to theoretically describe processes, like
$\tau$\-/decays into hadrons, by matching the quantum numbers of the
fields, we choose in specifying the two\-/point function, to the
outgoing hadrons. We will see, that the two\-/point function $\Pi(q^2)$ is
related to hadronic states, by poles for $q^2 > 0$. Here \textsc{np}\-/effects
become important and we need to introduce the \textsc{ope}, which handles
\textsc{np} parts as \textsc{qcd}\-/condensates. These condensates are
remainders of the \textsc{qcd}\-/vacuum $\Omega_{QCD}$, which in contrast to the
normal\-/ordered products of field in $\textsc{qed}$, do not vanish, but remain
as parameters and have to be phenomenological fitted or calculated by other
...
with developing the \define{fesr}{finite energy sum rules}, which we will apply
to extract the strong coupling from tau\-/decays into hadrons.
...
\end{equation}
The components of the triplet are the colours\footnote{The colour
...
\end{figure}
The Feynman\-/rules are the standard tool to make theoretical predictions in any
\textsc{qft}. Unfortunately the Feynman\-/rules are a \textsc{pt} tool and do
not always give the correct results in \textsc{qcd}, as we will see in the
following section.
...
\end{equation}
\Cref{eq:RGE} is referred to as \define{rge}{renormalisation group equation} and
...
the anomalous mass dimension is responsible for the running of the quark masses.
...
that it \textit{runs} by varying its scale $\mu$. This has some important
implications:
\begin{itemize}
\item The strong coupling we want to extract has different values at different
  scales. In the literature (e.g. \cite{pdg2016}) \(\alpha_s\) is commonly
  compared at the \(Z\)\-/boson scale of around \SI{91}{\giga\eV}. As we a
  ...
  \cite{Straub2016}.
\item The strong coupling becomes strong for low energies. It reaches a critical
  value at
  \begin{equation}
    \alpha_s(\siunits{1}{\giga\eV}) \approx 0.5,
  \end{equation}
  which questions the applicability of \textsc{pt} for energies lower than
  \siunits{1}{\giga\eV} (as seen from the grey zone in \cref{fig:runningOfAs}).
\item It leads to confinement and asymptotic freedom. Asymptotic freedom states,
  that for high energies (small distances), the strong coupling becomes
  diminishing small and quarks and gluons do not interact. Thus in isolated
  baryons and mesons the quarks are separated by small distances, move freely
  and do not interact. On the other hand we are not able to separate the quarks
  in a meson or baryon. No quark has been detected as single particle yet. This
  is qualitatively explained with the gluon field carrying colour charge. These
  gluons form so-called \textit{flux-tubes} between quarks, which cause a
  ...
  quark\-/antiquark pair. As a result we will probably never be able to observe
  an isolated quark. This phenomenon is referred to as colour confinement or
  simply confinement.
\end{itemize}
\begin{figure}
...
\end{figure}
To display the running of the strong coupling we integrate the
\(\beta\)\-/function
\begin{equation}
...
behaviour is known, as the previously mentioned, \textit{asymptotic freedom} and
\textit{confinement}.
...
\siunits{1.776}{\giga\electronvolt}. To deal with the upper integration limit
...
\define{fesr}{finite energy sum rules} and introduces a energy cutoff. We thus
...
\end{equation}
Unfortunately we still cannot theoretically evaluate \define{rhs}{right\-/hand
  side} as the line integral includes the singularities of the spectral
...
\textsc{qcdsr}, which where of greate use to us \cite{Narison1989,Rafael1997,Colangelo2000,Dominguez2013}.
\end{document}
