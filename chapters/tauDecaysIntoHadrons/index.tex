...
and \textsc{ckm} factors from now on. \Cref{eq:hadronicTauDecayRatio} was first
...
through so\-/called \define{dv}{duality violations}, which are well known
...
% This leads to two possible definitions of the \textsc{bs}, depending if we
% move the contour above or below the singularity, which is referred to as
% ``ambiguity of the Borel\-/integral''.
...
\end{align}
which have been defined in section five of \cite{Beneke2008}. As the large
order behaviour of the Adler function is governed by a sign\-/alternating
\textsc{uv} renormalon divergence and the lower\-/orders are not, only the
leading \textsc{uv} singularity has been included. Furthermore the intermediate
orders are governed by \textsc{ir} renormalons. Thus the first two (\(m=2\) and
