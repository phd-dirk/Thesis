...
\cref{eq:standardLorentzDecomposition} to display the
...
\end{equation}
The two\-/point functions in \cref{eq:tauDecayRatioTL} can now be replaced
...
Fixing the number of colours to \(N_c=3\) the missing coefficients up to order four
in \(\alpha_s\) read:
\begin{equation}
...
During this work we will insert the known invariant quark condensates (see \cref{table:constants}) as
...
operators has been calculated in \cite{Lanin1986} and leads to a large amount
