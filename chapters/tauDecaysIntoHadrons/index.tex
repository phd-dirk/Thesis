...
permits one of the most precie determinations of the strong coupling $\alpha_s$.
The inclusive $\tau$-decay ratio
\begin{equation}
  \label{eq:inclusiveRatio}
  R_\tau = \frac{\Gamma(\tau \to \nu_\tau + \text{Hadrons})}{\Gamma(\tau \to \nu_\tau e^+ e^-)}
\end{equation}
can be precisly calculated and is sensitive to $\alpha_s$. Due to the mass of
the $\tau$-lepton $m_\tau\approx\SI{1.8}{\giga\electronvolt}$ it is a good
phenomen to perform low-energy QCD analysis.
...
of the \textit{optical theorem} can be taken from \cite{Schwab2002}):
\begin{equation}
...
\end{equation}
The $\tau$-decay ratio depends on the two-point function
\begin{equation}
  \Pi_{\mu\nu,ij}^{V/A}(s) \equiv i \int \dif x \,e^{ipx} \langle \Omega | T \{ J_{\mu,ij}^{V/A}(x) J_{\nu,ij}^{V/A}(0)^\dagger \} | \Omega \rangle,
\end{equation}
with $|\Omega\rangle$ being the physical vacuum. The vectorial and axial-vector
currents are given by
\begin{equation}
  J_{\mu,ij}^{V}(x) = \anti{q}_j(x) \gamma_\mu q_i(x) \quad \text{and} \quad J_{\mu,ij}^{A}(x) = \anti{q}_j(x) \gamma_\mu \gamma_5 q_i(x)
\end{equation}
where $i,j$ stand for the light quark flavours u, d and s.

The general correlator $\Pi^{\mu\nu}(q^2)$ can be decomposed into a vector/
axial-vector (V/A) and scalar/ pseudo-scalar (S/P) part containing a correction \cite{Broadhurst1975}
\begin{equation}
...
term are corrections arising due to the physical vacuum $|\Omega\rangle$.

The general correlator $\Pi^{\mu\nu}$ can also be decomposed into transversal
...
To relate the two different contributions we note, that only the scalar components of
\cref{eq:correlatorVectorScalarDecomposition} carry a mass term. Using the
\textit{Ward identity}
\begin{equation}
  q_\mu \Pi^{\mu\nu}(q^2) = 0
\end{equation}
we can introduce two four-momenta into
\cref{eq:correlatorVectorScalarDecomposition}
\begin{equation}
  q_\mu q_\nu \Pi^{\mu\nu}(q^2) = (m_i \mp m_j)^2 \Pi^{S,P}(q^2) + (m_i \mp m_j)[\langle \anti{q}_i q_i \rangle \mp \langle \anti{q}_j q_j \rangle]
\end{equation}
to relate the longitudinal of \cref{eq:correlatorTransversalLongitudinalDecomposition}
\begin{equation}
  q_\mu q_\nu \Pi^{\mu\nu}(q^2) = q^4 \Pi^{(L)}(q^2) = s^2 \Pi^{(L)}(s),
\end{equation}
where we defined $s \equiv q^2$. Thus
\begin{equation}
  s^2 \Pi^{(L)}(s) = (m_i \mp m_j)^2 \Pi^{(S,P)}(s) + (m_i \mp m_j)[ \langle \anti{q}_i q_i \rangle \mp \langle \anti{q}_j q_j \rangle].
\end{equation}
Furthermore we can relate the transversal and vectorial components via
\begin{equation}
  ...
  \end{split} 
\end{equation}
where $\Pi^{(T+L)}(s) \equiv \Pi^{(T)}(s) + \Pi^{(L)}(s)$, such that
\begin{equation}
  \Pi^{(V,A)}(s) = \Pi^{(T)}(s) + \Pi^{(L)} = \Pi^{(T+L)}.
\end{equation}

Inspecting the inclusive ratio $R_\tau$ in \cref{eq:inclusiveRatio} introduces a
problematic integral over the real axis of $\Pi(s)$ from $0$ up to $m_\tau$.
The integral is problematic for two reasons:
\begin{itemize}
...
calculated beyond leading order in \cite{Lanin1986} 
vaccuum saturation approximation \cite{Beneke2008,Braaten1991,Shifman1978}
\begin{equation}
  \left. D_{ij,V/A}^{(1+0)}(s) \right\rvert_{D=6} = \pm \frac{32}{3} \pi^2 a_\mu \frac{\rho_{V/A}^{(6)}}{s^3} \langle \anti{q}_i q_i \rangle \langle \anti{q}_j q_j \rangle = \frac{32}{7} \pi^2 a_\mu \frac{\rho_{V/A}^{(6)}}{s^3} \left( \langle \anti{q}_i q_i \rangle^2 + \langle \anti{q}_i q_i \rangle \right)
\end{equation}

\begin{equation}
  \left. D_{ij,V/A}^{(1+0)} \right\rvert_{D=8} = \frac{\rho_{V/A}^{(8)}}{s^4}
\end{equation}
