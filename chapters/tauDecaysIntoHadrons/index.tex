...
$m_\tau$\-/scale. The fits are separated corresponding to the used weight. Every
weight contains multiple fits for different $s_0$\-/momenta. We will start with
the kinematic weight, which appears naturally in the inclusive $\tau$\-/decay ratio.

\subsection{Kinematic weight: $\omega(x) = (1-x)^2(1+2x)$}
The kinematic weight is defined as $\omega(x) = (1-x)^2(1+2x)$. Its
polinomial contains terms proportional to $x^2$ and $x^3$ which makes it
sensitive to the dimension six and eight OPE contributions. The fits have
been performed within the framework of FOPT for different numbers of $s_0$.
The momentum sets are characterized by its lowest energy $s_{min}$. We
fitted values up to $\SI{1.5}{\giga\eV}$. Going to lower energies is
questionable due to the coupling constant becoming to large, which implies a
breakdown of PT and appearing DVs. Furthermore it bares the
...
the OPE. For the fitting\-/parameters $\alpha_s, c_6$ and $c_8$ we have given the results in
...
$\chi^2$ per \textit{degree of freedom} \textsc{dof} and fits with lower
$s_{min}$ have a too low $\chi^2$ per \textsc{dof}. We achieved three excellent,
close to one $\chi^2$ per \textsc{dof} values. Furthermore the obtained values
for the strong coupling and dimension six and eight OPE contributions agree
within error boundaries, which underlines the stability of the fit. The best fit
