...
\end{equation}
where $\Ima \Pi$ is imaginary part of the two-point function we introduced in
\cref{sec:twoPointFunction}. Applying Cauchy's theorem, as seen in
\cref{eq:qcdSumRules}, to the \cref{eq:hadronicTauDecayRatio} we get
\begin{equation}
  \label{eq:rTauT+L}
  R_\tau = 6 \pi i \oint_{s=m_\tau} \frac{\dif s}{m_\tau^2}
  \left( 1 - \frac{s}{m_\tau^2} \right)
  \left[ \left( 1 + 2 \frac{s}{m_\tau^2} \right) \Pi^{(T)}(s) + \Pi^{(L)}(s) \right].
\end{equation}
...
\begin{tcolorbox}[ams equation,myformula]
  ...
  R_\tau = - \pi i \oint_{\abs{s}=m_\tau^2} \frac{\dif x}{x} (1 - x)^3 \left[ 3
    (1 + x) D^{(L+T)}(m_\tau^2 x) + 4 D^{(L)}(m_\tau^2 x) \right],
\end{tcolorbox}
...
\section{Computation of \(R_\tau\)}
The previously derived expression for the tau decay ratio can be further
organised in different \(\delta\) contributions
\begin{equation}
...
\end{equation}
where \(\delta^{(0)}\) is the perturbative contribution, \(\delta^{(EW)}\) is a
electroweak correction, \(\delta^{(DV)}\) is the duality violation correction
and \(\delta^{(D)}\) are the higher dimensional contributions from the
\textsc{ope}. Note that in this work we will assume \(delta^{(DV)}\) to be
negligible small for the weights we will apply.

We now want to derive the theoretical expressions needed to calculate every
contribution to \cref{eq:rTauContributions} starting with the perturbative
contribution.


\subsection{The perturbative contribution}
We will treat the correlator in the chiral limit for which the longitudinal
components $\Pi^{L}(s)$ of \cref{eq:longitudinalCorrelator} vanish. Thus the
axial and vectorial contributions are equal. In the massless case we then can
write the vector correlation function $\Pi(s)$ as a sum over different orders of
...
\begin{tcolorbox}[ams equation,myformula]
  D_V^{(T+L)} = - s \od{\Pi_V^{(T+L)}(s)}{s} = \frac{N_c}{12 \pi^2}
...
reduce its numbers. But as we will see in the following section the \textsc{rge}
gives us two different choices in the order of the computation of the
perturbative contribution to the inclusive tau decay ratio.
...
\cref{eq:rTauContributions}) in the chiral limit, such that $D^{(L)}$ vanishes
as
\begin{equation}
  \label{eq:rTauDelta0}
...
Using CIPT we can sum the logarithms by setting the scale to $\mu^2 = -m_\tau^2
...
\end{equation}
which selects adler function coefficients $c_{n,1}$ with a fixed $k=1$. Handling
the logarithms left us with the integration of $\alpha_s(- m_\tau^2 x)$ over the
closed-contour $\oint_{\abs{x}=1}$, which now depends on the integration
variable x. In general we have to decide if we want to perform a contour
integration with a constant coupling constant and variable logarithms (FOPT) or
``constant logarithms'' and a running coupling (CIPT).
...
As today we do not know if FOPT or CIPT is the correct approach of measuring
$\alpha_s$. Therefore there are currently three ways of stating results:
\begin{itemize}
