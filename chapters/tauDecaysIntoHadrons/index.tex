...
We can further introduce a scale\-/invariant gluon\-/condensate
\cite{Spiridonov1988}, which are combinations of the minimally subtracted
operators
\begin{equation}
  \beta_1 \langle a G^2 \rangle_I \equiv \beta(s) \langle
  G_{(a)}^{\mu\nu}G_{\mu\nu}^{(a)} \rangle + 4 \gamma(a) \sum_{i=u,d} \langle
  m_i \anti{q}_i q_i \rangle - \frac{3}{4\pi^2} \sum_{i,j=u,d} m_i^2m_j^2
  \gamma_0^{ij}(a).
\end{equation}
During this work we will state the invariant gluon condensate.
...
treatment of the \textsc{ope} we have to discuss possible duality
violations.
...
and the existence of \define{dv}{duality violations} is well known
\cite{Cata2008,Cata2009}. Experimental data shows, that the plot of the measured
total tau decay ratio has exponential decreasing, sinusoidal behaviour, that
cannot be reproduced by the \textsc{ope}. Consequently for the cases with
apparent \textsc{dv} we have to somehow include the corrections coming from
\textsc{dv} and adapt \cref{eq:rTauContributions}, leading to
\begin{equation}
...
that for fitting the kinematic weight in the \textsc{V}\-/channel, which is known to be
sensible for \textsc{dv} at lower energies \cite{Boito2011a}, we would have
seven parameters instead of only three. Making use of the model
...
\cref{fig:monomialWeightGraphs}). The farther we are away the better we are
...
double pinched weight is sufficient to neglect effects caused by duality
violation. Additionally to applying pinched weights we focus on combinations of
vector and axial\-/vector contributions, which as we will see now have visibly
suppressed \textsc{dv}.
...
has negative parity. A meson, like the pion particle, is a composite particle
consisting of a quark and an anti-quark. Consequently a single pion carries
...
The error propagation has been derived in \cite{Boito2011a,Boito2011} and is
given as
\begin{equation}
...
% LocalWords:  inclusiveRatio theoreticalBackground kinematicWeight dvModel
% LocalWords:  qcdSumRules tauDecayRatioT dispersionRelation myformula
% LocalWords:  tauDecayRationT perturbativeContributionFeynmanDiagrams
