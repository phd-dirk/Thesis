...
Let's rapidly show how the OPE contributions are calculated with a the
``standard example'' (following \cite{Pascual1986}), where we calculate the
perturbative and quark-condensate Wilson-coefficients for the $\rho$-meson. We
start with the two-point function 
\begin{equation}
  \Pi^{\mu\nu}(q) = i \int \dif^4 x e^{iqx} \langle \Omega | T\{ j^\mu(x) j^\nu(0) \} \rangle
\end{equation}
with two-quark currents
\begin{equation}
  j^\mu(x) = [\anti q_i \Gamma q_j](x).
\end{equation}
For the $\rho$-meson, which is composed of u and d quarks, the current takes the
following form
\begin{equation}
  j^\mu(x) = \frac{1}{2}\left(:[\anti{u} \gamma^\mu u](x) - \anti{d} \gamma^\mu d](x)\right).
\end{equation}
The correlator tensor can be rewritten, due to its Lorentz structure, into a
scalar by using the \textit{Ward identity}
\begin{equation}
  \Pi^{\mu\nu}(s) = (q^\mu q^\nu - q^2 g^{\mu\nu}) \Pi(s).
\end{equation}
\begin{figure}
...
\end{figure}
In \cref{fig:OPEFeynmanDiagram} we draw the Feynman-diagram to at tree level,
from which we can take the uncontracted mathematical expression
\begin{equation}
  \begin{split}
