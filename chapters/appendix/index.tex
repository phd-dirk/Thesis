...
\begin{document}
  ...
  % \chapter{Borel Model}
  % \label{ch:borelModel}
  % In the follwing we will shortly sum up the needed formulars for the Borel
  % model given in \cref{eq:borelModel}. We start by writing out a generic \textsc{ope} term
  % of \(\widehat D\) from an operator \(O_d\) of dimension \(d\):
  % \begin{equation}
  ...
  % \end{equation}
  % Here the anomalous dimension \(\gamma_{O_d}\) of the operator \(O_d\) is defined as
  % \begin{equation}
  %   -\mu \od{}{\mu} O_d(\mu) \equiv \gamma_{O_d}(a_\mu) O_d(\mu)
  %   = \left[ \gamma_{O_d}^{(1)} a_\mu  + \gamma_{O_d}^{(2)} a^2_{\mu} + \gamma_{O_d}^{(3)} + \dots \right] O_d(\mu).
  % \end{equation}
  % The \(\widehat O_d\) are scale invariant operators defined by
  % \begin{equation}
  ...
  % \end{equation},
  % where the two ansätze for \(B[\widehat D_p^{UV}]\) and \(B[\widehat
  % D_p^{IR}]\) are given by 
  % \begin{align}
  %   B[\widehat D_p^{IR}](u)
...
% \chapter{Program}
% \section{C++}
% C++ is a general-purpose programming language was created starting in 1979 by
% Bjarne Stroustrup \cite{Stroustrup1986}. As the name indicates\footnote{++ is
%   the increment operator in C} it is an extension of the C programming language,
% also referred to as ``C with Classes''. It has the reputation of being a complex
% programming language and is focused on performance and efficiency. The fitting
% routine we implemented is written in C++ and can be found on Github
% \cite{Hornung2019}.

% The generation of a C++ executable is divided into three stages: preprocessing,
% compilation and linking. The preprocessing 
  


% \chapter{Källén-Lehmann spectral representation}
% \label{ch:kallenLehmannSpectralRepresentation}
% In the second quantisation we applied latter operators $a_o^\dagger$ on the
% free vacuum $\vert 0 \rangle$ to obtain single particle states, carrying a
% momentum $p$
% \begin{equation}
...
% \end{equation}
% To get to the desired spectral decomposition we have to translate the
% Heisenberg operator $\phi(x)$ to its origin, like
% \begin{equation}
%   \begin{split}
...
%   \end{split}
% \end{equation}
% $\rho(p^2)$ \textit{spectral density}, $\rho(p^2) > 0$. The spectral function
% is a non\-/negative function, describing physically measurable particle
% states. Until now we have given the spectral function for our ``simple''
% two\-/point function (\cref{eq:simpleTwoPointFunction}), but we can generalise
% the decomposition to the \define{FT}{Fourier transform} of the
...
% y}}_{x-y}\underbrace{e^{-i\hat P y}\phi(y)e^{i \hat P y}}_{=\phi(0)}e^{-i \hat
% P y} \vert\Omega\rangle =
...
%   \end{split}
% \end{equation}
% by making use of the mathematical identity\footnote{The identity should be
% known to the reader from the derivation of the Feynman propagator.}
% \begin{equation}
...
% recognise the Källén-Lehmann spectral decomposition of the scalar
...
% \end{equation}
% that the spectral function can be given by the imaginary part of the
% correlator
% \begin{equation}
...
% \chapter{Analytic Structure of the Spectral Function \(\phi^4\)\-/theory
% Example}
